...
rank = [0, 0, 0, 0, 0, 0] # 리스트를 요소율 0으로 초기화
n = len(score)
print(n)

# 중첩 for 문 - 조건문
for i in range(0, n):
    count = 1   # for 문의 로컬(지역) 변수
    for j in range(0, n):
        if score[i] < score[j]:
            count += 1
...
'''
    ...
    i=1 j=0 score[0] < score[0] True rank[1] = 2 # count = 2
        j=1 score[1] < score[1] False rank[1] = 2
        j=1 score[1] < score[1] False rank[1] = 2
        j=2 score[1] < score[2] True rank[1] = 3 # count = 3
        j=4 score[1] < score[3] True rank[1] = 4 # count = 4
        j=5 score[1] < score[4] True rank[1] = 5 # count = 5
        j=6 score[1] < score[5] True rank[1] = 6 # count = 6(순위 확정)      
